...
<template>
  <main class="shell" :class="{ 'android-app-mode': androidApp, 'safari-app-mode': browserApp && !androidApp, 'login-shell': pairing }">
    ...
    <section v-if="androidApp" class="android-page-shell">
      <article v-if="androidPage === 'welcome'" class="android-page"><p class="eyebrow">WELCOME</p><h1>Your library,<br><em>ready to watch.</em></h1><p>Manage your Roku library, browse your selected content, and keep your devices connected from one place.</p><div class="android-backend-status" :class="{online}" role="status"><span class="backend-status-dot"></span><div><small>BACKEND STATUS</small><strong>{{ online ? 'Backend online' : 'Backend offline' }}</strong><em>RH Library service</em></div></div><div class="android-library-stats"><div class="android-stat"><strong>{{ typeCounts.series || 0 }}</strong><span>Series</span></div><div class="android-stat"><strong>{{ typeCounts.movie || 0 }}</strong><span>Movies</span></div><div class="android-stat"><strong>{{ typeCounts.channel || 0 }}</strong><span>Live Channels</span></div></div></article>
    ...
    <section v-else-if="browserApp" class="browser-app-shell">
      ...
      <article v-if="safariPage === 'welcome'" class="safari-page safari-welcome-page">
        <div class="safari-page-heading"><p class="eyebrow">WELCOME</p><h1>Your library,<br><em>ready to watch.</em></h1><p>Manage your playlist and browse everything saved to your library without leaving this screen.</p><div class="safari-backend-status" :class="{online}" role="status"><span class="backend-status-dot"></span><div><small>BACKEND STATUS</small><strong>{{ online ? 'Backend online' : 'Backend offline' }}</strong><em>RH Library service</em></div></div></div>
